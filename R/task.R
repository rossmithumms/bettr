...
#' debugging but clutter up the job server log files.
#' Note that if `suppress_task_errors` is set to FALSE (the default),
#' a job with a task error will never return a result.
#' Defaults to FALSE.
#' @param suppress_logging Boolean Suppresses the logging
#' of the subprocess standard out and error. Defaults to FALSE.
#' @param suppress_task_errors Boolean Indicates whether the
#' calling Rscript or command (usually invoked by the job server)
#' should exit with an error code status (and thus alertfully break
#' on the job server). All tasks will still be attempted in order.
#' Defaults to TRUE.
#' @param stop_on_first_error Boolean Indicates whether the first
#' error generated by any job task stops the entire job or whether
#' the job continues to try subsequent tasks.  Defaults to FALSE.
#' @export
run_next_job_in_queue <- function(
  ...
  for (i in seq_len(nrow(next_bettr_job))) {
    ...
    if (length(errors) > 0 && stop_on_first_error) {
      message("... job called with stop_on_first_error == TRUE; stopping")
      break
    }

    if (length(errors) > 0 && !suppress_task_errors) {
      message("... job called with suppress_task_errors == FALSE; stopping")
      break
    }
  }
